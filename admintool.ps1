######################################################################
# Original Author: Rich Prescott
# Heavily Modified By: Dylan Branson
...
if (Invoke-Command -ComputerName $computername -ScriptBlock {get-module -list ActiveDirectory}) {$RSAT = "No"}
        else {$RSAT = "No"}
...
Function ADImport{
if($true){Import-Module ActiveDirectory}
